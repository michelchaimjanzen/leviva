...
export function TargetRunnerEngine({ configuredSlides, forcedMode, onComplete }: TargetRunnerProps) {
  ...
  const lockedScrollRef = useRef<number>(0); 
    ...
    setTimeout(() => {
      if (activeSlide.slideType === 'graded' && activeSlide.selectionMode === 'multiple') {
        ...
        if (savedScrollPosition !== undefined) {
          ...
          window.scrollTo({ top: lockedScrollRef.current, behavior: 'auto' });
        }
      }
    }, 50);

  }, [currentIndex, activeSlide.id, activeSlide.slideType, activeSlide.selectionMode]);
